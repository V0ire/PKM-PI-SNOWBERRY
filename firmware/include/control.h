...
struct ManualCommand {
  bool valid = false;
  ManualTarget target = ManualTarget::UNKNOWN;
  Mode mode = Mode::AUTO;
  bool state = false;
...
struct PumpStartRecord {
  int64_t epoch_ms = 0;
  uint32_t boot_ms = 0;
};
using PumpStartReserver = bool (*)(const PumpStartRecord&);
void setPumpStartReserver(PumpStartReserver reserver);
